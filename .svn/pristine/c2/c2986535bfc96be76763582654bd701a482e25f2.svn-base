...
public class ApprovalReq{
	...
	public void validate()
	{
		if(StringUtils.isBlank(this.getId())){
    		throw new InnerException(BizError.PARAM_NOT_EXIST, "id is empty.");
    	}
    	if(StringUtils.isBlank(this.getApplyName())){
    		throw new InnerException(BizError.PARAM_NOT_EXIST, "apply_name is empty.");
    	}
    	if(StringUtils.isBlank(this.getApplyTime())){
    		throw new InnerException(BizError.PARAM_NOT_EXIST, "apply_time is empty.");
    	}
    	if(StringUtils.isBlank(this.getReceiveName())){
    		throw new InnerException(BizError.PARAM_NOT_EXIST, "receive_name is empty.");
    	}
    	if(StringUtils.isBlank(this.getReceiveAccount())){
    		throw new InnerException(BizError.PARAM_NOT_EXIST, "receive_account is empty.");
    	}
    	if(0 == this.getTotalFee()){
    		throw new InnerException(BizError.PARAM_NOT_EXIST, "total_fee is 0.");
    	}
	}
}
